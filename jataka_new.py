...
tithi_name = ['PADYA','BIDIGE','TADIGE','CHOUTHI','PANCHAMI','SHASTI','SAPTAMI','ASTAMI','NAVAMI','DASHAMI','YEKADASHI','DWADASHI','TRAYODASI','CHATURDASI','HUNNIME',
    'PADYA','BIDIGE','TADIGE','CHOUTHI','PANCHAMI','SHASTI','SAPTAMI','ASTAMI','NAVAMI','DASHAMI','YEKADASHI','DWADASHI','TRAYODASI','CHATURDASI','AMAVASYE']
...
def bhava():
    ...
    for i in range(1,12):
        ...
        if beg_bhav[i] > 360.0:
            beg_bhav[i] -= 360.0
        i += 1
    print("Ayanamsa     : %7.2f"%plnt[0])
    print("Obliquity    : %7.2f"%obliq)
    print("Sideral Time : %7.2f\n\n"%sidtime)
    print("     BHAVA POSITION")
    print(dash*45)
    print("Bhava No.        Bhava Begin     Mid Bhava")
    print(dash*45)
    for i in range(0,12):
        print("%2d               %7.2f           %7.2f"%(i+1,beg_bhav[i],mid_bhav[i]))
    print(dash*45)
...
def misc():
    ...
    ra = int(rasi)
    tt[3] = rasi
    print("Tithi            : %-7.2f          Tithi Name          : %s"%(tt[0],tithi_name[ti]))
    print("Nakshatra        : %-7.2f          Nakshatra Name      : %s"%(tt[1],nakshatra_name[na]))
    #print("Yoga             : %-7.21f          Yoga Name           : %s"%(tt[2],yoga_name[ti]))
    print("Rasi             : %-7.2f          Rasi Name           : %s"%(tt[3],rasi_name[ra]))
    print('\n')
    dasa_remaining()
    print(dasa_remain)
...
def saptavarga_table():
    print('\n            SAPTAVARGAS TABLE')
    print(dash*65)
    print('PLANET   DIV.1    DIV.3    DIV.7    DIV.9    DIV.12    DIV.16')
    print(dash*65)
...
def varga_calc(m):
    ...
    for z in range(1,34):
        if z%8==1 and z!=17:
            ...
            print('* %s %s * %s %s * %s %s * %s %s *' %(tz[z][0],tz[z][1],tz[z][2],tz[z][3],tz[z][4],tz[z][5],tz[z][6],tz[z][7]))
        else:
            print('* %s %s *                       * %s %s *' %(tz[z][0],tz[z][1],tz[z][6],tz[z][7]))
        z += 1
...
def planet_position():
    for i in range(1,13):
        ...
        if (bb < aa):
            ...
            plan_pos = 'DIR'
        print('%s      %7.2f      %2d      %-12s       %d      %s'%(graha[i],aa,a,nakshatra_name[b],c,plan_pos))
        i += 1
...
def vimst(a,b,c):
    ...
    e = a + 100.0
    b = a - p
    print('                 VIMSOTTARI DASA')
    print(dash*50)
    print(' DASA         BHUKTI      DATE   MONTH   YEAR')
    print(dash*50)
    for c in range(q,q+9):
        if c>8:
            c-=9
        for d in range(0,9):
            n = c+d
            ...
            if r==0:
                ...
                t -= 28
                r = 3
            print(' %-7s      %-7s       %2d    %2d     %4d'%(x1[c],x1[n],t,r,p))
            d += 1
        f = c+1
        if f>8:
            f -= 9
        print('***** %10s DASA ENDS *****'%x1[c])
        print('')
        if b>e:
            break
        print('*****%10s DASA STARTS *****'%x1[f])
        print('')
        c += 1
    print(dash*50)
    print('NOTE - THE DATES GIVEN ARE FOR DASA ENDING DATES.')

# MAIN
def jataka():
    global co, ret, longt, lat, JD, h6

    name = input("Enter Name : ")
    dob = input("Enter Date of Birth in format DD.MM.YYYY (Leave blank to use default value): ") or '27.6.1990'
    tob = input("Enter Time of Birth in 24-hour format HH.MM (Leave blank to use default value): ") or '13.35'
    pob = input("Enter Place of Birth : ")
    latitude = input("Enter Latitude in format deg-mt (Leave blank to use default value): ") or '22-34'
    longitude = input("Enter Longitude in format deg-mt (Leave blank to use default value): ") or '88-24'
    ...
    print('\n')
    print('Name             : %s' %name)
    print('Date of Birth    : %1s.%1s.%1s' %(dttm.strftime("%d"),dttm.strftime("%m"),dttm.strftime("%y")))
    print('Day of Birth     : %s' %dttm.strftime("%A"))
    print('Time of Birth    : %1s-%1s %2s IST' %(dttm.strftime("%I"),dttm.strftime("%M"),dttm.strftime("%p")))
    print('Place of Birth   : %s' %pob)
    print('Latitude         : %3s' %latitude)
    print('Longitude        : %3s' %longitude)
    misc()
    print('\n\n')
    print('             PLANETARY POSITION')
    dash = '-'
    print(dash*70)
    print('PLANET    LONGITUDE    RASI    NAKSHATRA NAME    PADA    DIRECT/RET')
    print(dash*70)
    planet_position()
    print(dash*70)
    print('\n\n')
